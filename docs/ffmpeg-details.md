...
Each entry works in its own directory:

```
<export directory>/tmp/run_<microseconds>/
    f_000000.mp4
    f_000001.mp4
    ...
    list.txt
...
Preferences shows how much space the scratch directory is using and offers to
empty it. Only `<export>/tmp` is ever touched.

> The Electron app reused one shared `tmp` directory and never cleaned it up.

---

## Output naming

The output goes to the export directory, named after the source with the chosen
container's extension. If that name is already taken — or would be the source
file itself — a counter is appended: `lecture (1).mp4`.
